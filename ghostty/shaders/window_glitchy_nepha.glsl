...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    distortion *= gnoise(vec3(0., y * .02, t * 250.)) * (glitchAmount * 2. + .005);

    ++displayNoise;
    distortion += smoothstep(.999, 1., sin((uv.y + t * 1.6) * 2.)) * .011;
    distortion -= smoothstep(.999, 1., sin((uv.y + t) * 2.)) * .011;
    ...
    col += (.15 + .65 * glitchAmount) * (hash33(vec3(fragCoord, mod(float(iFrame),
					1000.))).r) * displayNoise;
    col -= (.25 + .75 * glitchAmount) * (sin(4. * t + uv.y * iResolution.y * 1.15))
					* displayNoise;
